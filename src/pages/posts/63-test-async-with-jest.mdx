...
<span style={{backgroundColor:"#67c8cf22"}}>**단위 테스트**란 작은 코드 조각을, 빠르게, 격리된 방식으로 검증하는 자동화된 테스트</span>를 말합니다. 그런데 위 테스트는 '격리된' 환경에서 진행되지 않는 테스트입니다. `execute`가 실행하는 `fetchList` 함수가 외부 API를 호출하기 때문에, 외부 의존성이 생겨버렸기 때문입니다.
...
export const meta = PostUtil.createPostMeta({
    ...
    description: "비동기 작업이 포함된 함수를 테스트해봅시다",
    category: '',
    series: "",
    tags: ["jest", "test", "글또"],
    postedAt: '2023.03.26.',
