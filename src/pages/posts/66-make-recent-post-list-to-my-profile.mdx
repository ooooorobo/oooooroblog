...
# [act](https://github.com/nektos/act) - 로컬에서 액션 실행하기

깃허브 액션 워크플로우 작성은 한 번만에 성공하는 법이 없는 것 같다. 그렇다고 액션을 테스트하려고 마구 커밋을 날리다 보면…

![Untitled](/image/66/commit-list.png)

![Untitled](/image/66/action-list.png)

정신을 차리고 보면 커밋 목록과 액션 실행 목록이 이렇게 돼버린다. 이런 사태를 방지하려면 [act](https://github.com/nektos/act)를 사용해 로컬에서 액션을 충분히 테스트하는 게 좋다.

<WavyLine size="4"/>

act는 [링크](https://github.com/nektos/act#installation-through-package-managers)를 참고해서 환경에 따라 설치하면 된다. act는 도커를 사용하기 때문에, 도커도 설치해줘야 한다. ([도커 설치 링크](https://www.docker.com/)) 

설치가 완료되면, 액션을 실행할 레포지토리 루트에서 `act` 커맨드를 실행하면 된다. 

실행 가능한 액션 목록은 `act -l` 커맨드로 확인할 수 있다.

```bash
➜  ooooorobo git:(master) ✗ act -l
WARN  ⚠ You are using Apple M-series chip and you have not specified container architecture, you might encounter issues while running act. If so, try running it with '--container-architecture linux/amd64'. ⚠
Stage  Job ID             Job name           Workflow name          Workflow file          Events
0      make_recent_posts  make_recent_posts  make-recent-posts.yml  make-recent-posts.yml  repository_dispatch
```

이 중에서 실행을 원하는 액션을 `act -j [job_name]` 커맨드로 실행하면 된다.

<br/>

그런데 `repository_dispatch` 웹훅을 통해 실행되는 액션의 경우, 추가로 필요한 데이터를 받아와야 한다. 이럴 경우 `—-event_path` 옵션으로 데이터를 넘겨줄 수 있다.

먼저, 데이터를 담는 json 파일을 작성한다. 이 데이터는 curl 커맨드의 -d 옵션 값에 주는 데이터를 그대로 넣어주면 된다.
...
```bash
act -j make_recent_posts -e event-data.json
```

이 값은 github actions workflow에서와 마찬가지로, `github.event` 값을 통해 접근할 수 있다.

![Untitled](/image/66/run-act.png)
